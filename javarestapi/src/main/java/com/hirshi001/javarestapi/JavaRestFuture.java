...
public class JavaRestFuture<T, U> implements RestFuture<T, U> {
    ...
    private Queue<JavaRestFuture<U, ?>> nextFutures;
    private Queue<ListenerExecutor<T, U>> listenerExecutors;
    private Queue<RestFutureListener<T, U>> listeners;
    private Consumer<Throwable> onFailure;


    public JavaRestFuture(ScheduledExec executor, boolean cancellable, RestFutureConsumer<T, U> task, RestFuture<?, T> parent) {
    ...
    protected <B> RestFuture<U, B> add(RestFutureConsumer<U, B> function) {
        JavaRestFuture<U, B> future = new JavaRestFuture<>(executor, false, function, this);
